export const homepageData = [
  {
    id: 110022,
    type: "landingpage",
    theme: "theme-midnight-green",

    
    nav: [
      {
        id: 11002,
        theme: "theme-midnight-green"
      }
    ],


    hero: [
      {        
        id: 212,
        theme: "theme-text-light-grey",
        type: "frontpage",
        headline: "Vi brygger viden for en oplyst fremtid",
        text: "Carlsbergfondet støtter fri grundforskning inden for naturvidenskab, humaniora og samfundsvidenskab.",
        img: [
          {
            id: 1010122,
            src: "/data/media/gettywhales.png",
            alt: "hero image"
          }
        ],
        video: [
          {
            id: 119993311,
            src: "/data/media/videos/herovid.mp4",
            alt: "hero"
          }
        ],
        link: [
          {
            id: 122,
            text: "Ansøg om bevilling",
            url: "/ansog"
          }
        ]
      }
    ],


    txtBlock: [
      {
        id: 213,
        headline: "Husk at ansøge før 1 oktober",
        headline2nd: "",
        subheadline: "",
        textSec: [
          {
            id: 12,
            text: "Ønsker du at få del i Carlsbergfondets midler til grundforskning inden for naturvidenskab, samfundsvidenskab og humaniora?"
          },
          {
            id: 13,
            text: "Dyk ned i fondets efterårsopslag, der tilbyder en lang række virkemidler rette mod såvel yngre postdocs som etablerede forskere på lektor professorniveau."
          }
        ],
        text: "Ønsker du at få del i Carlsbergfondets midler til grundforskning inden for naturvidenskab, samfundsvidenskab og humaniora? Dyk ned i fondets efterårsopslag, der tilbyder en lang række virkemidler rette mod såvel yngre postdocs som etablerede forskere på lektor professorniveau.",
        img: [
          {
            id: 111,
            src: "/data/media/Bryggeren5.jpg",
            alt: "calendar image"

          }
        ],
        link: [
          {
            id: 122,
            text: "Søg nu",
            url: "/ansog"
          }
        ]
      }
    ],


    txtCtaSection: [
      {
        id: 21983,
        headline: "Frist:",
        headline2nd: "1 Oktober",
        subheadline: "Ønsker du at få del i Carlsbergfondets midler til grundvidenskabelig forskning inden for naturvidenskab, samfundsvidenskab og humaniora?",
        text: "Dyk ned i fondets Efterårsopslag 2022, der tilbyder en lang række virkemidler rettet mod såvel yngre postdocs som etablerede forskere på lektor -og professorniveau. Frist: 1 oktober 2022.",
        img: [
          {
            id: 111,
            src: "/data/media/calendar.svg",
            alt: "calendar image"

          }
        ],
        link: [
          {
            id: 122,
            text: "Se alle åbne forslag",
            url: "/ansog"

          }
        ]
      }
    ],


    txtSliderSection: [
      {
        id: 214,
        tag: "",
        headline: "Nyheder",
        headline2nd: "",
        // subheadline: "Nyt om projekter, ansøgninger og alt det andet spændende der er relateret til og omkring Carlsberg-fondets arbejde.",
        text: "",
        link: [
          {
            id: 122,
            text: "Flere nyheder",
            url: "/nyheder"

          }
        ],
        articles: [
          {
            id: 312,
            online: "y",
            type: "article",
            styleTag: "styletag",
            tag: "news",
            category: "news",
            headline: "Indlandsis, hvalfangere og jordbrug kan blandt andet stå her som en overskrift.",
            text: "Lorem ipsum dolor sit amet",
            createdDate: "12.11.2022",
            updatedDate: "12.11.2022",
            link: [
              {
                id: 2220,
                txt: "Læs mere",
                url: "/nyheder",
                type: "read"
              }
            ],
            img: [
              {
                id: 3121,
                src: "/data/media/article1.png",
                alt: "Citat img"
              }
            ]
          },
        
          {
            id: 233,
            online: "y",
            type: "article",
            styleTag: "styletag",
            tag: "news",
            category: "news",
            headline: "Carlsbergfondet får en ny direktør",
            text: "Lorem ipsum dolor sit amet",
            createdDate: "12.11.2022",
            updatedDate: "12.11.2022",
            link: [
              {
                id: 2220,
                txt: "Læs mere",
                url: "/nyheder",
                type: "read"
              }
            ],
            img: [
              {
                id: 2331,
                src: "/data/media/direktør1.png",
                alt: "img"
              }
            ]
          },
        
          {
            id: 647,
            online: "y",
            type: "article",
            styleTag: "styletag",
            tag: "news",
            category: "news",
            headline: "Månedens forsker: Jeg har været facineret af detaljen",
            text: "Lorem ipsum dolor sit amet",
            createdDate: "12.11.2022",
            updatedDate: "12.11.2022",
            link: [
              {
                id: 2220,
                txt: "Læs mere",
                url: "/nyheder",
                type: "read"
              }
            ],
            img: [
              {
                id: 6471,
                src: "/data/media/personnewsimg.png",
                alt: "img"
              }
            ]
          },
        
          {
            id: 222,
            online: "y",
            type: "article",
            styleTag: "styletag",
            tag: "news",
            category: "news",
            headline: "En artikel omkring et gennembrud eller en opdagelse",
            text: "Lorem ipsum dolor sit amet",
            createdDate: "12.11.2022",
            updatedDate: "12.11.2022",
            link: [
              {
                id: 2220,
                txt: "Læs mere",
                url: "/nyheder",
                type: "read"
              }
            ],
            img: [
              {
                id: 2221,
                src: "/data/media/hedgehog.png",
                alt: "News article"
              }
            ]
          },
        
          {
            id: 267,
            online: "y",
            type: "article",
            styleTag: "styletag",
            tag: "news",
            category: "news",
            headline: "Et forskningsprojekt der handler om bevarelsen af verdenshavene ",
            text: "Lorem ipsum dolor sit amet",
            createdDate: "12.11.2022",
            updatedDate: "12.11.2022",
            link: [
              {
                id: 2220,
                txt: "Læs mere",
                url: "/nyheder",
                type: "read"
              }
            ],
            img: [
                {
                  id: 2221,
                  src: "/data/media/gettywhales.png",
                  alt: "News article"
                }
              ]
          }
        ]
      }
    ],


    txtEventSection: [
      {
        id: 27775,
        tag: "Kalender",
        headline: "Begivenheder 2023",
        headline2nd: "2023",
        subheadline: "",
        text: "",
        img: [
          {
            id: 255,
            src: "",
            alt: ""

          }
        ],
        link: [
          {
            id: 422,
            text: "Flere begivenheder",
            url: "/"
          }
        ],
        articles: [
          {
            id: 53,
            tag: "Foredrag",
            headline: "Kom til en aften i selskab med hekse, gryder og koste.",
            dayMonth: "17.02",
            year: "2023",
            img: [
              {
                id: 530,
                src: "/data/media/witchesimg.png",
                alt: "witches"
              }
            ],
            link: [
              {
                id: 531,
                text: "Mere info",
                url: ""
              }
            ]
          },
          {
            id: 54,
            tag: "Kulturnatten: Åbent hus",
            headline: "Vi åbner de ellers lukkede døre til kælderen på Glyptotektet.",
            dayMonth: "12.05",
            year: "2023",
            img: [
              {
                id: 540,
                src: "/data/media/sculpture.png",
                alt: "fungus"
              }
            ],
            link: [
              {
                id: 541,
                text: "Mere info",
                url: ""
              }
            ]
          },
          {
            id: 55,
            tag: "Foredrag",
            headline: "Kom med til en aften i selskab med hekse, gryder og koste.",
            dayMonth: "16.03",
            year: "2023",
            img: [
              {
                id: 550,
                src: "/data/media/Mask-group2.svg",
                alt: "Lines"
              }
            ],
            link: [
              {
                id: 551,
                text: "Mere info",
                url: ""
              }
            ]
          }
        ]
      }
    ],

    
    txtInfoSection: [
      {
        id: 296,
        tag: "Hvad støtter Carlsbergfondet",
        headline: "420 Mio. gode grunde til at søge støtte.",
        link: [
          {
            id: 2962,
            text: "Hvem kan søge?",
            url: "/"
          }
        ],
        categories: [
          {
            id: 123,
            name: "01. Naturvidenskab"
          },
          {
            id: 124,
            name: "02. Humanistisk videnskab"
          },
          {
            id: 125,
            name: "03. Samfundsvidenskab"
          }
        ],
        articles: [
          {
            id: 33,
            tag: "01. Naturvidenskab",
            headline: "Naturvidenskab",
            text: [
              {
                id: 134,
                column: "Naturvidenskab. Carlsbergfondet støtter skelsættende og visionær forskning med over 420 mio. kr. årligt. Forskning som gør os klogere, løser globale problemer og som alle har glæde af."
              },
              {
                id: 234,
                column: "Carlsbergfondet støtter skelsættende og visionær forskning med over 420 mio. kr. årligt. Forskning som gør os klogere, løser globale problemer og som alle har glæde af."
              }
            ],
            link: [
              {
                id: 331,
                text: "Mere info",
                url: "/ansog"
              }
            ]
          }          
        ]
      }
    ],


    purposeSection: [
      {
        id: 222999,
        tag: "",
        headline: "Fondets formål",
        headline2nd: "",
        subheadline: "",
        textSec: [
          {
            id: 8832,
            headline: "Udbredelse",
            text: "Gennem en bestemmende aktiepost at sikre værdiskabelsen i Carlsberg A/S, udbredelse af Carlsberg som et anerkendt ølmærke samt udvikle ølbrygning til den størst mulige fuldkommenhed.",
            img: [
              {
                id: 121213,
                src: "/data/media/udbredelse.svg",
                alt: "Udbredelse svg"
              }
            ]
          },
          {
            id: 9933,
            headline: "Støtte",
            text: "Uddeling af aktier i Carlsberg A/S til velgørende formål ved at virke til fremme af og støtte for naturvidenskaberne samt matematik og filosofi, de humanistiske videnskaber og samfundsvidenskaberne.",
            img: [
              {
                id: 121213,
                src: "/data/media/støtte.svg",
                alt: "Støtte svg"
              }
            ]
          }
        ],
        link: [
          {
            id: 12912,
            text: "Om fondet",
            url: "/omfondet"
          }
        ],
      }
    ],


    videoSection: [
      {
        id: 31175,
        tag: "",
        headline: "Watch & learn",
        headline2nd: "",
        subheadline: "Gå på opdagelse i vores lærerige videocontent og kom med når topforskere fortæller om den seneste viden og viser eksempler hvor spændende videnskab er.",
        text: "Relaterede videoer",
        link: [
          {
            id: 9342,
            text: "Flere videoer",
            url: "/nyheder"
          }
        ],
        articles: [
          {
            id: 113,
            online: "y",
            type: "video",
            styleTag: "feature",
            tag: "Talk",
            category: "category",
            headline: "Uden galakser ville vi slet ikke eksistere",
            text: "Der har ikke altid eksisteret galakser. De er opstået i løbet af universets historie. Galakser bestsår af en form for fortætning af “mørkt stof” og gas. Tror Professor Johan Fynbo på at der er liv på andre planeter?",
            time: "10:37",
            createdDate: "12.11.2022",
            updatedDate: "12.11.2022",
            img: [
              {
                id: 1330,
                src: "/data/media/vidarticle1.png",
                alt: "img"
              }
            ],
            video: [
              {
                id: 988332,
                src: "/data/media/videos/galaxer.mp4",
                alt: "video",
                img: "/data/media/vidarticle1.png"
              }
            ],
            link: [
              {
                id: 5951,
                text: "Afspil",
                url: "",
                type: "play"
              }
            ]
          },        
          {
            id: 5224,
            online: "y",
            type: "video",
            styleTag: "regular",
            tag: "Talk",
            category: "category",
            headline: "På jagt efter spike proteinets akilleshæl",
            text: "Der har ikke altid eksisteret galakser. De er opstået i løbet af universets historie. Galakser bestsår af en form for fortætning af “mørkt stof” og gas. Tror Professor Johan Fynbo på at der er liv på andre planeter?",
            time: "03:53",
            createdDate: "12.11.2022",
            updatedDate: "12.11.2022",
            img: [
              {
                id: 620,
                src: "/data/media/vidarticle2.png",
                alt: "img"
              }
            ],
            video: [
              {
                id: 92824,
                src: "/data/media/videos/greenland1.mp4",
                alt: "video",
                img: "/data/media/vidarticle2.png"
              }
            ],
            link: [
              {
                id: 661,
                text: "Afspil",
                url: "",
                type: "play"
              }
            ]
          },        
          {
            id: 532,
            online: "y",
            type: "video",
            styleTag: "regular",
            tag: "Talk",
            category: "category",
            headline: "Indlandsisen vil smelte hurtigere mod 2100 end hidtil antaget.",
            text: "Der har ikke altid eksisteret galakser. De er opstået i løbet af universets historie. Galakser bestsår af en form for fortætning af “mørkt stof” og gas. Tror Professor Johan Fynbo på at der er liv på andre planeter?",
            time: "03:53",
            createdDate: "12.11.2022",
            updatedDate: "12.11.2022",
            img: [
              {
                id: 8360,
                src: "/data/media/vidarticle3.png",
                alt: "img"
              }
            ],
            video: [
              {
                id: 87824,
                src: "/data/media/videos/science.mp4",
                alt: "video",
                img: "/data/media/vidarticle3.png"
              }
            ],
            link: [
              {
                id: 853,
                text: "Afspil",
                url: "",
                type: "play"
              }
            ]
          },        
          {
            id: 995,
            online: "y",
            type: "video",
            styleTag: "regular",
            tag: "Talk",
            category: "category",
            headline: "På jagt efter spike proteinets akilleshæl",
            text: "Der har ikke altid eksisteret galakser. De er opstået i løbet af universets historie. Galakser bestsår af en form for fortætning af “mørkt stof” og gas. Tror Professor Johan Fynbo på at der er liv på andre planeter?",
            time: "03:53",
            createdDate: "12.11.2022",
            updatedDate: "12.11.2022",
            img: [
              {
                id: 5141,
                src: "/data/media/vidarticle4.png",
                alt: "img"
              }
            ],
            video: [
              {
                id: 11137324,
                src: "/data/media/videos/greenland2.mp4",
                alt: "video",
                img:  "/data/media/vidarticle4.png"
              }
            ],
            link: [
              {
                id: 3921,
                text: "Afspil",
                url: "",
                type: "play"
              }
            ]
          },
          {
            id: 29276,
            online: "y",
            type: "video",
            styleTag: "regular",
            tag: "Talk",
            category: "category",
            headline: "På jagt efter spike proteinets akilleshæl",
            text: "Der har ikke altid eksisteret galakser. De er opstået i løbet af universets historie. Galakser bestsår af en form for fortætning af “mørkt stof” og gas. Tror Professor Johan Fynbo på at der er liv på andre planeter?",
            time: "03:53",
            createdDate: "12.11.2022",
            updatedDate: "12.11.2022",
            img: [
              {
                id: 5141,
                src: "/data/media/vidarticleimg.png",
                alt: "img"
              }
            ],
            video: [
              {
                id: 11137324,
                src: "/data/media/videos/greenland1.mp4",
                alt: "video",
                img: "/data/media/vidarticle4.png",
              }
            ],
            link: [
              {
                id: 3921,
                text: "Afspil",
                url: "",
                type: "play"
              }
            ]
          },
          {
            id: 51922,
            online: "y",
            type: "video",
            styleTag: "regular",
            tag: "Talk",
            category: "category",
            headline: "Indlandsisen vil smelte hurtigere mod 2100 end hidtil antaget.",
            text: "Der har ikke altid eksisteret galakser. De er opstået i løbet af universets historie. Galakser bestsår af en form for fortætning af “mørkt stof” og gas. Tror Professor Johan Fynbo på at der er liv på andre planeter?",
            time: "03:53",
            createdDate: "12.11.2022",
            updatedDate: "12.11.2022",
            img: [
              {
                id: 8360,
                src: "/data/media/vidarticle3.png",
                alt: "img"
              }
            ],
            video: [
              {
                id: 87824,
                src: "/data/media/videos/science.mp4",
                alt: "video",
                img: "/data/media/vidarticle3.png"
              }
            ],
            link: [
              {
                id: 853,
                text: "Afspil",
                url: "",
                type: "play"
              }
            ]
          },        
          {
            id: 21915,
            online: "y",
            type: "video",
            styleTag: "regular",
            tag: "Talk",
            category: "category",
            headline: "På jagt efter spike proteinets akilleshæl",
            text: "Der har ikke altid eksisteret galakser. De er opstået i løbet af universets historie. Galakser bestsår af en form for fortætning af “mørkt stof” og gas. Tror Professor Johan Fynbo på at der er liv på andre planeter?",
            time: "03:53",
            createdDate: "12.11.2022",
            updatedDate: "12.11.2022",
            img: [
              {
                id: 5141,
                src: "/data/media/vidarticle4.png",
                alt: "img"
              }
            ],
            video: [
              {
                id: 11137324,
                src: "/data/media/videos/greenland2.mp4",
                alt: "video",
                img:  "/data/media/vidarticle4.png"
              }
            ],
            link: [
              {
                id: 3921,
                text: "Afspil",
                url: "",
                type: "play"
              }
            ]
          },
          {
            id: 991188446,
            online: "y",
            type: "video",
            styleTag: "regular",
            tag: "Talk",
            category: "category",
            headline: "På jagt efter spike proteinets akilleshæl",
            text: "Der har ikke altid eksisteret galakser. De er opstået i løbet af universets historie. Galakser bestsår af en form for fortætning af “mørkt stof” og gas. Tror Professor Johan Fynbo på at der er liv på andre planeter?",
            time: "03:53",
            createdDate: "12.11.2022",
            updatedDate: "12.11.2022",
            img: [
              {
                id: 5141,
                src: "/data/media/vidarticleimg.png",
                alt: "img"
              }
            ],
            video: [
              {
                id: 11137324,
                src: "/data/media/videos/greenland1.mp4",
                alt: "video",
                img: "/data/media/vidarticle4.png",
              }
            ],
            link: [
              {
                id: 3921,
                text: "Afspil",
                url: "",
                type: "play"
              }
            ]
          }
        ]

      }
    ],


    newsletterBlock: [
      {
        id: 2138276,
        tag: "Nyhedsbrev",
        headline: "Får du seneste nyt?",
        headline2nd: "",
        subheadline: "Hvis du vil være på forkant med hvad der rør sig indenfor videnskab, forskning og kultur, kan du tilmelde dig vores nyhedsbrev lige herunder.",
        text: "Hvis du vil være på forkant med hvad der rør sig indenfor videnskab, forskning og kultur, kan du tilmelde dig vores nyhedsbrev lige herunder.",
        inputfields: [
          {
            id: 1212,
            type: "name",
            text: "Navn"
          },
          {
            id: 1922,
            type: "email",
            text: "Email"
          }
        ],
        img: [
          {
            id: 11184411,
            src: "/data/media/Bryggeren5.jpg",
            alt: "calendar image"

          }
        ],
        link: [
          {
            id: 122647,
            text: "Tilmeld",
            url: ""
          }
        ]
      }
    ],


    podcastSection: [
      {
        id: 184382,
        tag: "Podcast",
        headline: "Kom med på en lytter",
        headline2nd: "",
        subheadline: "",
        text: "",
        link: [
          {
            id: 422,
            text: "Flere podcasts",
            url: ""
          }
        ],
        articles: [
          {
            id: 3145,
            tag: "Serie",
            headline: "Fra kaskelothvaler i dybhavet til flagermus i natten",
            episodes: "8 episoder",
            img: [
              {
                id: 5250,
                src: "/data/media/pod2.png",
                alt: "billede"
              }
            ],
            link: [
              {
                id: 531,
                text: "Link",
                url: ""
              }
            ]
          },
          {
            id: 2955,
            tag: "Serie",
            headline: "Hvalfangere, rensdyrjægere og jordbrugere i 4500 års grønlandsk historie",
            episodes: "10 episoder",
            img: [
              {
                id: 190,
                src: "/data/media/pod1.png",
                alt: "billede"
              }
            ],
            link: [
              {
                id: 551,
                text: "Link",
                url: ""
              }
            ]
          }
        ]
      }
    ]


  }
]